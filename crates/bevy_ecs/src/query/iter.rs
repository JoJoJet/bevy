...
use super::{ReadOnlyWorldQuery, WorldQueryFilter};

/// An [`Iterator`] over query results of a [`Query`](crate::system::Query).
///
/// This struct is created by the [`Query::iter`](crate::system::Query::iter) and
/// [`Query::iter_mut`](crate::system::Query::iter_mut) methods.
pub struct QueryIter<'w, 's, Q: WorldQuery, F: WorldQueryFilter> {
    tables: &'w Tables,
...
impl<'w, 's, Q: WorldQuery, F: WorldQueryFilter> QueryIter<'w, 's, Q, F> {
    /// # Safety
...
impl<'w, 's, Q: WorldQuery, F: WorldQueryFilter> Iterator for QueryIter<'w, 's, Q, F> {
    type Item = Q::Item<'w>;
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        ...
        let min_size = if F::IS_ARCHETYPAL { max_size } else { 0 };
        (min_size, Some(max_size))
    }
}

// This is correct as [`QueryIter`] always returns `None` once exhausted.
impl<'w, 's, Q: WorldQuery, F: WorldQueryFilter> FusedIterator for QueryIter<'w, 's, Q, F> {}
...
pub struct QueryManyIter<'w, 's, Q: WorldQuery, F: WorldQueryFilter, I: Iterator>
where
...
impl<'w, 's, Q: WorldQuery, F: WorldQueryFilter, I: Iterator> QueryManyIter<'w, 's, Q, F, I>
where
    I::Item: Borrow<Entity>,
{
    /// # Safety
...
impl<'w, 's, Q: ReadOnlyWorldQuery, F: WorldQueryFilter, I: Iterator> Iterator
...
impl<'w, 's, Q: ReadOnlyWorldQuery, F: WorldQueryFilter, I: Iterator> FusedIterator
    for QueryManyIter<'w, 's, Q, F, I>
where
    I::Item: Borrow<Entity>,
{
...
impl<'w, 's, Q: ReadOnlyWorldQuery, F: WorldQueryFilter, const K: usize> Iterator
    for QueryCombinationIter<'w, 's, Q, F, K>
{
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        ...
        let min_combinations = if F::IS_ARCHETYPAL { known_max } else { 0 };
        (min_combinations, max_combinations)
    }
}

impl<'w, 's, Q: WorldQuery, F: WorldQueryFilter> ExactSizeIterator for QueryIter<'w, 's, Q, F>
where
    F: ArchetypeFilter,
{
    fn len(&self) -> usize {
...
impl<'w, 's, Q: ReadOnlyWorldQuery, F: WorldQueryFilter, const K: usize> FusedIterator
    for QueryCombinationIter<'w, 's, Q, F, K>
{
}
...
impl<'w, 's, Q: WorldQuery, F: WorldQueryFilter> QueryIterationCursor<'w, 's, Q, F> {
