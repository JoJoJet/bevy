...
pub trait HierarchyQueryExt<'w, 's, Q: WorldQuery, F: WorldQueryFilter> {
...
impl<'w, 's, Q: WorldQuery, F: WorldQueryFilter> HierarchyQueryExt<'w, 's, Q, F>
    for Query<'w, 's, Q, F>
{
    fn iter_descendants(&'w self, entity: Entity) -> DescendantIter<'w, 's, Q, F>
    where
...
///
/// Traverses the hierarchy breadth-first.
pub struct DescendantIter<'w, 's, Q: WorldQuery, F: WorldQueryFilter>
where
    Q::ReadOnly: WorldQuery<Item<'w> = &'w Children>,
{
...
impl<'w, 's, Q: WorldQuery, F: WorldQueryFilter> DescendantIter<'w, 's, Q, F>
where
    Q::ReadOnly: WorldQuery<Item<'w> = &'w Children>,
{
...
impl<'w, 's, Q: WorldQuery, F: WorldQueryFilter> Iterator for DescendantIter<'w, 's, Q, F>
where
    Q::ReadOnly: WorldQuery<Item<'w> = &'w Children>,
{
    type Item = Entity;
...
pub struct AncestorIter<'w, 's, Q: WorldQuery, F: WorldQueryFilter>
where
    Q::ReadOnly: WorldQuery<Item<'w> = &'w Parent>,
{
...
impl<'w, 's, Q: WorldQuery, F: WorldQueryFilter> AncestorIter<'w, 's, Q, F>
where
    Q::ReadOnly: WorldQuery<Item<'w> = &'w Parent>,
{
...
impl<'w, 's, Q: WorldQuery, F: WorldQueryFilter> Iterator for AncestorIter<'w, 's, Q, F>
where
    Q::ReadOnly: WorldQuery<Item<'w> = &'w Parent>,
{
    type Item = Entity;
